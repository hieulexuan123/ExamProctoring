...
def processItems():
    ...
    print('Time for processing items', (time.time()-start_time)*1000)
...
def processFaces(face_data):
    ...
    if len(faces)==0:
        ...
        print('is match', face_data['is_match'])
    ...
    with frame_array.get_lock():  # Ensure exclusive access
        np.copyto(np.frombuffer(frame_array.get_obj(), dtype=np.uint8), frame.flatten())
    print('Time for processing faces', (time.time()-start_time)*1000)
...
def verify_face(face_data, user):
    spoof_detector = FAS()
    while True:
        start_time = time.time()
...
def proctor(user):
    ...
    verify_process = multiprocessing.Process(target=verify_face, args=(face_data, user))
